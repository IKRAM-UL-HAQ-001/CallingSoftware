                            ...
                            <table id="DataTable" class="table align-items-center mb-0 table-striped table-hover px-2">
                                ...
                                <tbody id="DataTableBody">
                                    @foreach ($PhoneNumbers as $phoneNumber)
                                        <tr>
                                            <td style="width: 45%;" class="encrypted-data" id="phoneNumber" data-id="{{ $phoneNumber->id }}">{{ $phoneNumber->phone_number }}
                                            </td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $phoneNumber->user->name }}
                                            </td>
                                            <td style="width: 45%;" class="encrypted-data">
                                                {{ $phoneNumber->user->created_at }}
                                            </td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-warning"
                        data-action="{{ route('exchange.demo_send.formPost') }}">Demo Send</button>
                    <button type="button" class="btn btn-warning"
                        data-action="{{ route('exchange.reject.formPost') }}">Reject</button>
                    <button type="button" class="btn btn-warning"
                        data-action="{{ route('exchange.refer_id.formPost') }}">Refer ID</button>
                    <button type="button" class="btn btn-success"
                        data-action="{{ route('exchange.new_id.formPost') }}">Customer</button>
                    <button type="button" class="btn btn-info"
                        data-action="{{ route('exchange.follow_up.formPost') }}">Follow Up</button>
                    <button type="button" class="btn btn-dark"
                        data-action="{{ route('exchange.complaint.formPost') }}">Complaint</button>
                    <button type="button" class="btn btn-primary"
                        data-action="{{ route('exchange.walk.formPost') }}">Walk</button>
